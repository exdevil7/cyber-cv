...
export const SummarySection = ({ summary }: SummarySectionProps) => {
    ...
    return (
        <div className="lg:w-3/5 w-full flex-shrink-0 flex flex-col">
            {/* System HUD Status Bar (Condensed with Glow) */}
            <div className="flex flex-nowrap gap-6 lg:gap-10 mb-8 border-y border-retro-cyan/10 py-3 overflow-x-auto no-scrollbar relative group terminal-glow">
                ...
                <div className="px-4 border-l-4 border-retro-cyan flex flex-col whitespace-nowrap relative z-10">
                    <span className="text-[9px] font-black font-orbitron text-retro-cyan/40 uppercase tracking-[0.2em]">Exp_Level</span>
                    <span className="text-sm font-bold font-mono text-retro-cyan">06_Y+</span>
                </div>
                <div className="px-4 border-l-4 border-retro-purple flex flex-col whitespace-nowrap relative z-10">
                    <span className="text-[9px] font-black font-orbitron text-retro-pink/40 uppercase tracking-[0.2em]">Engine</span>
                    <span className="text-sm font-bold font-mono text-retro-pink">JAVA_SPRING</span>
                </div>
                <div className="px-4 border-l-4 border-retro-pink flex flex-col whitespace-nowrap relative z-10">
                    <span className="text-[9px] font-black font-orbitron text-retro-purple/40 uppercase tracking-[0.2em]">Status</span>
                    <span className="text-sm font-bold font-mono text-retro-purple italic uppercase tracking-tighter">OPEN_TO_WORK</span>
                </div>

                {/* Option B: Message HUD Segment */}
                <div className="px-4 border-l-4 border-retro-pink flex flex-col whitespace-nowrap relative z-10 group/msg cursor-pointer" onClick={() => setIsMessageOpen(!isMessageOpen)}>
                    <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-black font-orbitron uppercase tracking-[0.2em] transition-colors ${isMessageOpen ? 'text-retro-pink' : 'text-retro-pink/40 group-hover/msg:text-retro-pink/80'}`}>Message</span>
                        {!isMessageOpen && (
                            <span className="w-1.5 h-1.5 bg-retro-pink rounded-full animate-ping"></span>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
                        <MessageSquare size={14} className={isMessageOpen ? 'text-retro-pink' : 'text-retro-pink/60 group-hover/msg:text-retro-pink'} />
                        <span className={`text-sm font-bold font-mono transition-colors ${isMessageOpen ? 'text-retro-pink' : 'text-white/60 group-hover/msg:text-white'}`}>
